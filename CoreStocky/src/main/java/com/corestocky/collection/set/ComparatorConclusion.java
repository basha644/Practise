...
//	Comparable vs Comparator:
		/*For predefined Comparable classes default natural sorting order is already available if we are not satisfied with
		default natural sorting order then we can define our own customized sorting order by Comparator.
		For predefined non-Comparable classes [like StringBuffer] default natural sorting order is not available
		...
		For our own classes [like Customer, Student, and Employee] we can define default natural sorting order by using Comparable interface.
		 The person who is using our class,
